			...
			try{
				ytd_menu_popup_renderer.addEventListener('yt-popup-opened', function() {
					function add_customization_menuitem() {
						...
						try{
							video_block_title =  selected_grid_media.querySelector('a#avatar-link').title;// 合輯的值會是'undefined'
						}catch(e){
							// 點擊 short 的會有例外問題(short的結構不同)
				...
				console.log('[YOPF]: 插入事件成功!');
				is_done = true;
				document.body.removeEventListener("click", add_popup_rendere_event);
			}catch(e){
				console.log('[YOPF]: 插入事件失敗，警報: '+ e.name);
				console.log(e.message);
			}
		}else{
			console.log('[YOPF]: 仍在等待使用者點擊那三個點點...');
		}
	}

	//預設中 Youtube 不會把先把彈出選項元素建構好，要等使用者點擊彈出選項才會建構(每次有氣泡點擊事件就偵測一次彈出選項存不存在)
	document.body.addEventListener('click', add_popup_rendere_event);
    console.log('[YOPF]: YOPF載入完成，正等待使用者點擊那三個點點');
